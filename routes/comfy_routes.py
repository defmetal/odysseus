...
Generic, discovery-driven Image/Video composer modes. Hidden when
``comfy_base_url`` is unset or the ComfyUI server is unreachable.
Music generation is a separate ``/api/music/*`` surface.
...
class LoraParam(BaseModel):
    ...
    # Optional LoRA category from config/comfy_loras.json ("style" | "character").
    # Declared so Pydantic does not drop it as an extra field.
...
class GenerateParams(BaseModel):
    ...
    # config/comfy_models.json preset key. When set (image kind only),
    ...
    # override -- "pick a model, type a prompt, hit Generate". Unset means
    # "no preset" -- the caller is responsible for unet/clip/vae/loras.
...
def configured_comfy_base_url() -> str:
    from src.settings import get_setting
    return (get_setting("comfy_base_url", "") or "").strip()


def validate_comfy_base_url(url: str) -> str:
    """Require a configured URL and run SSRF checks before any outbound call."""
    cleaned = (url or "").strip()
    if not cleaned:
        raise HTTPException(status_code=503, detail="ComfyUI is not configured. Set comfy_base_url in Settings.")
    ok, reason = check_outbound_url(cleaned)
    if not ok:
        raise HTTPException(status_code=400, detail=f"comfy_base_url failed SSRF checks: {reason}")
    return cleaned.rstrip("/")


def _client() -> ComfyClient:
    return ComfyClient(validate_comfy_base_url(configured_comfy_base_url()))
...
def _workflows_dir() -> Path:
    return Path(DATA_DIR) / "comfy" / "workflows"
...
def _read_loras_registry() -> dict:
    """Raw config/comfy_loras.json "loras" dict (key -> entry),
    ...
    don't duplicate it")."""
    path = Path(__file__).resolve().parent.parent / "config" / "comfy_loras.json"
    if not path.is_file():
        path = Path(DATA_DIR) / "comfy" / "loras.json"
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
...
def _load_curated_loras() -> list[dict]:
    """Enabled entries from config/comfy_loras.json (plan §6.3a)."""
    out = []
...
def _load_video_model_presets() -> tuple[dict, list[dict]]:
    ...
    flattened_list) shape, reading config/comfy_models.json's
...
# config/comfy_models.json. Before this, /api/comfy/options parsed it
...
async def _resolve_input_image(input_image: str) -> tuple[str, str, str]:
    ...
    (folder_paths.get_annotated_filepath(), ComfyUI/
...
def _write_user_turn(
    ...
    `prompt` is the text the user typed (the caller passes
    job["user_prompt"] from body.params.prompt).
...
def setup_comfy_routes() -> APIRouter:
    ...
    async def comfy_status(request: Request):
        ...
        if not url:
            return {
                "ok": False, "available": False, "reason": "unset",
                "version": "", "queue_pending": 0, "queue_running": 0,
                "error": "comfy_base_url is not set",
            }
        ok, reason = check_outbound_url(url)
        if not ok:
            return {
                "ok": False, "available": False, "reason": "ssrf",
                "version": "", "queue_pending": 0, "queue_running": 0,
                "error": reason,
            }
        try:
            info = await ComfyClient(url).status()
            return {
            ...
            return {
                "ok": False, "available": False, "reason": "unreachable",
                "version": "", "queue_pending": 0, "queue_running": 0,
                "error": str(e),
            }
    ...
    async def comfy_generate(body: GenerateRequest, request: Request):
        ...
        if body.kind == "image":
            # Model preset (config/comfy_models.json) resolution --
            # MUST run before build_image_graph() (apply_model_preset() can
            # fill unet/clip/clip_type/vae/loras). Image kind only: a video
            # request's `model` is resolved against `video_models` below.
            ...
            # Video model preset (config/comfy_models.json's
            # `video_models` section) -- same "pick a model, the recipe
            # follows" idea as the image block above. An absent `model` key
            # skips this block so Wan keeps working unchanged
            # (video_engine stays at its "wan22_i2v" default).
        ...
        # Keep the user's typed prompt separately from resolved_params.
