...
class Skip(Exception):
    ...
    tests below that depend on optional local files which may not exist
    in a fresh clone."""
...
def test_n_lora_chaining_rewires_links():
    loras = [
        {"comfy_name": "style_lora_v1/style_lora_v1.safetensors", "weight": 0.75},
        {"comfy_name": "character_lora_v1/character_lora_v1.safetensors", "weight": 1.0},
...
def test_identical_seed_and_randomize_false_produces_byte_identical_graphs():
    ...
    params = {
        "prompt": "generic style, demo, a hero on a cliff",
...
def test_round_trip_txt2img():
    params = {
        "prompt": "generic style, demo, a hero on a cliff at sunset",
        "negative_prompt": "bad hands, extra fingers",
        "loras": [
            {"comfy_name": "style_lora_v1/style_lora_v1.safetensors", "weight": 0.8},
            {"comfy_name": "character_lora_v1/character_lora_v1.safetensors", "weight": 1.0},
        ],
...
def test_filter_safetensors_drops_junk():
    ...
    live = ["character_lora_v1.safetensors", "optimizer.pt", "style_lora_v1.safetensors", "README.md"]
    assert filter_safetensors(live) == [
        "character_lora_v1.safetensors", "style_lora_v1.safetensors",
    ]
...
def test_resolve_lora_entries_exact_match():
    ...
    live = ["style_lora_v1/style_lora_v1.safetensors", "some_other.safetensors"]
    entries = [{"key": "style_lora_v1", "comfy_name": "style_lora_v1/style_lora_v1.safetensors"}]
    out = resolve_lora_entries(entries, live)
    assert out[0]["available"] is True
    assert out[0]["resolved_name"] == "style_lora_v1/style_lora_v1.safetensors"
    assert out[0]["key"] == "style_lora_v1"  # other keys preserved verbatim
...
def test_resolve_lora_entries_basename_match_uses_the_live_spelling():
    ...
    live = ["character_lora_v1.safetensors", "style_lora_v1.safetensors"]
    entries = [
        {"key": "character_v1", "comfy_name": "character_lora_v1/character_lora_v1.safetensors"},
        {"key": "style_lora_v1", "comfy_name": "style_lora_v1/style_lora_v1.safetensors"},
    ]
    out = resolve_lora_entries(entries, live)
    assert out[0]["available"] is True
    assert out[0]["resolved_name"] == "character_lora_v1.safetensors"
    assert out[1]["available"] is True
    assert out[1]["resolved_name"] == "style_lora_v1.safetensors"


def test_resolve_lora_entries_unavailable_when_no_match_at_all():
    live = ["some_unrelated_file.safetensors"]
    entries = [{"key": "character_v1", "comfy_name": "character_lora_v1/character_lora_v1.safetensors"}]
    out = resolve_lora_entries(entries, live)
...
def test_apply_model_preset_fills_unset_fields_from_preset():
    preset = {
        ...
        "loras": [{"key": "style_lora_v1", "weight": 1.0}],
        "defaults": {
            "steps": 30, "cfg": 4.5, "sampler": "res_multistep", "scheduler": "simple",
    ...
    assert out["loras"] == [{"key": "style_lora_v1", "weight": 1.0}]
    assert out["steps"] == 30
    assert out["cfg"] == 4.5
...
def test_apply_model_preset_empty_negative_prompt_not_refilled():
    ...
    # prompt": ""}, zimage_general-shaped preset)["negative_prompt"] must stay
...
def test_preset_to_builder_empty_negative_prompt_survives_into_graph():
    ...
    assert "" in negs, negs
    assert "text, letters, lettering, logo, watermark, signature" not in negs, negs
...
def test_resolve_model_entries_all_present_is_available():
    entries = [{"key": "zimage_general", "unet": "z_image_bf16.safetensors",
    ...
    assert out[0]["key"] == "zimage_general"  # other keys preserved verbatim
...
def test_resolve_model_entries_basename_fallback_resolves():
    ...
    entries = [{"key": "zimage_general", "unet": "nested/z_image_bf16.safetensors",
                "clip": "qwen_3_4b.safetensors", "vae": "ae.safetensors"}]
    live = {"unets": ["z_image_bf16.safetensors"], "clips": ["qwen_3_4b.safetensors"], "vaes": ["ae.safetensors"]}
    out = resolve_model_entries(entries, live)
    assert out[0]["available"] is True
...
def test_resolve_model_entries_blank_field_is_not_missing():
    entries = [{"key": "zimage_general", "unet": "u.safetensors", "clip": "", "vae": ""}]
...
def test_qwen_preset_end_to_end_graph_shape():
    # Mirrors config/comfy_models.json's "qwen_image_2512" entry.
    preset = {
        "arch": "qwen_image",
...
def test_minimax_music3_graph_uses_official_nodes_not_h3_video():
    graph = build_minimax_music3_graph({
        "caption": "lofi beat, warm analog keys",
        "lyrics": "[Verse]\nhello world",
        "seconds": 45,
        "seed": 7,
    ...
    for expected in (
        ...
        assert expected in types, f"missing {expected} in Music 3 graph: {types}"
    assert "EmptyHunyuanVideoLatent" not in types
    assert "MiniMaxH3TextEncode" not in types
    encode = next(n for n in graph.values() if n["class_type"] == "MiniMaxMusic3TextEncode")
    assert encode["inputs"]["caption"] == "lofi beat, warm analog keys"
    assert encode["inputs"]["lyrics"] == "[Verse]\nhello world"
    assert encode["inputs"]["max_duration"] == 45
    assert encode["inputs"]["seed"] == 7
    latent = next(n for n in graph.values() if n["class_type"] == "EmptyMiniMaxMusic3LatentAudio")
    assert latent["inputs"]["seconds"] == 45
    recovered = introspect_graph(graph)["params"]
    assert recovered.get("caption") == "lofi beat, warm analog keys"
    assert recovered.get("lyrics") == "[Verse]\nhello world"


def test_negative_prompt_none_falls_back_to_empty_default():
    """None == caller said nothing -> empty generic negative."""
...
def test_negative_prompt_empty_string_is_honoured_not_replaced():
    """'' == caller explicitly wants NO negative prompt."""
...
# ---------------------------------------------------------------------------
# apply_model_preset() reused for VIDEO presets (config/
...
def test_resolve_model_entries_unet_low_and_audio_vae_absent_is_not_missing():
    ...
    entries = [{"key": "zimage_general", "unet": "u.safetensors", "clip": "c.safetensors", "vae": "v.safetensors"}]
