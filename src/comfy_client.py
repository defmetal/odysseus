...
Generic Image/Video/Music generation talks to a user-configured ComfyUI
server (settings key ``comfy_base_url``). Every
endpoint/message shape below was verified directly against the vendored
source in ComfyUI/ during this session (not just the plan's
summary of it):
...
class ComfyClient:
    ...
    def __init__(self, base_url: Optional[str] = None, *, timeout: float = 60.0):
        url = (base_url or DEFAULT_COMFY_BASE_URL or "").strip().rstrip("/")
        if not url:
            raise ComfyError("comfy_base_url is not configured")
        self.base_url = url
    ...
    async def upload_image(
        ...
        against ComfyUI/server.py's `image_upload()`
