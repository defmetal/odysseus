...
an optional model-preset registry; everything else is plain dict/list manipulation).
...
ComfyUI/ComfyUI/nodes.py; forcing denoise is meaningless for a
...
# Official ComfyUI Z-Image / SD3-family node schemas.
# ---------------------------------------------------------------------------
...
def comfy_image_ref(name: str, subfolder: str = "", folder_type: str = "input") -> str:
    ...
    Verified against ComfyUI/ComfyUI/nodes.py:1738-1739
...
def build_image_graph(params: dict) -> dict:
    ...
    # None == "caller said nothing" -> fall back to optional anti-text
    ...
    # Model-preset registry key (config/comfy_models.json): "lumina2"
...
def build_wan_i2v_graph(params: dict) -> dict:
    ...
    No style-prefix helper -- only the two fixed Wan lightning LoRAs
    (see _extract_wan_lora_weights()) are ever wired in.
...
#    MiniMax H3 video model files (not Music 3)
...
def build_minimax_h3_graph(params: dict) -> dict:
    ...
    above). Deliberately does NOT call any style-prefix helper and offers no
    LoRA stack at all -- like Wan, H3 has no knowledge of the optional style/
...
# checked directly against ComfyUI/ComfyUI/nodes.py's
...
# ---------------------------------------------------------------------------
# MiniMax Music 3 (local Comfy engine) -- NOT MiniMax H3 video
# ---------------------------------------------------------------------------
# Official ComfyUI nodes (comfy_extras/nodes_minimax_music.py):
#   MiniMaxMusic3TextEncode, EmptyMiniMaxMusic3LatentAudio
# Weights (Comfy-Org/MiniMax-Music-3): DiT + text encoder + DAV VAE.
# Hosted MiniMax Music 3 API is a separate backend (routes/music_routes.py).
...
# ---------------------------------------------------------------------------
#
# config/comfy_loras.json's `comfy_name` values were authored in the
# NESTED form ("style_lora_v1/style_lora_v1.safetensors") because ComfyUI
# may report either nested paths or bare filenames depending on how model
# roots are mapped. The functions below resolve exact then basename so a
# registry entry works against either live combo list.
...
def resolve_lora_entries(entries: Optional[list], live_names: Optional[list]) -> list[dict]:
    ...
    config/comfy_loras.json's curated registry rows AND a raw
    ...
    Three-way resolution, in order, per entry:
      ...
      2. Else, a BASENAME match (os.path.basename of the entry's comfy_name
         ...
         ("style_lora_v1/style_lora_v1.safetensors") resolve TODAY
         against a ComfyUI that only knows the bare filename
         ("style_lora_v1.safetensors"), and it is exactly what a
...
# ---------------------------------------------------------------------------
# G. load_model_registry / resolve_model_entries / apply_model_preset --
# config/comfy_models.json: "which model" as ONE user-facing choice
# ---------------------------------------------------------------------------
#
# config/comfy_models.json collapses unet+clip+clip_type+vae+loras+defaults
# into a single named preset so the Image tab is "pick a model, type a
# prompt, hit Generate". Pure, stdlib-only, degrade-don't-raise on a
# missing/malformed file.

_MODELS_REGISTRY_PATH = Path(__file__).resolve().parent.parent / "config" / "comfy_models.json"

# Used only if the registry file is missing/malformed so callers degrade
# instead of crashing. Generic first-available preset — no LoRAs required.
_FALLBACK_MODEL_REGISTRY = {
    ...
    "models": {
        "zimage_general": {
            "name": "Z-Image (base)",
            "group": "General",
            "description": "Plain Z-Image with no LoRAs.",
            "arch": "z_image",
            "unet": DEFAULT_UNET,
            "clip": DEFAULT_CLIP,
    ...
    # Wan's, the same bias as the image fallback (plain Z-Image, no LoRAs), built from this
...
def load_model_registry(path: Optional[str] = None) -> dict:
    """Read config/comfy_models.json -- the model-preset registry (see
...
def resolve_model_entries(entries: Optional[list], live: Optional[dict]) -> list[dict]:
    ...
    `entries` -- config/comfy_models.json's preset rows, flattened to
    ...
    SAME "vaes" live list -- config/comfy_models.json's `video_models`
...
# additions (reuse apply_model_preset() for config/
...
def apply_model_preset(params: Optional[dict], preset: Optional[dict]) -> dict:
    ...
    def _unset(key: str) -> bool:
        ...
        # refilled optional anti-text negative right back in, defeating
