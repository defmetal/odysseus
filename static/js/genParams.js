...
// generic Image/Video/Music composer modes for the design — this implements plan
...
// JUST the model picker (the connected Comfy catalog presets, via
// /api/comfy/options' `models`/`default_model`) + its description + a
// "pick a model, type a prompt, hit Generate." Everything that used to live in Simple (workflow, mode/img2img,
...
const IMAGE_BASELINE = Object.freeze({
  workflow: 'Custom',
  model: '',                  // discovery-only: first available catalog model after /api/comfy/options loads
...
const SIZE_PRESETS_IMAGE = ['1024x1024', '1152x864', '1216x672', 'custom'];
...
// Fields a MODEL PRESET (the connected Comfy catalog) can reseed on
...
// (e.g. Qwen's official aspect-ratio table vs MiniMax H3's 1344x768 vs
// Wan's 720x720) plus a 'custom'
...
// ── Model presets (the connected Comfy catalog, via /api/comfy/options'
...
// Video model presets (the connected Comfy catalog's `video_models`
...
function _modelModeBannerHtml(modelEntry) {
  if (!modelEntry || !modelEntry.description) return '';
  return `<div class="gen-params-hint">${esc(modelEntry.description)}</div>`;
}

// Video counterpart -- task item 4: "Show a one-line note that H3 generates
// synchronized audio." `has_audio` (the connected Comfy catalog) is true
...
function _renderVideoPanel(effective, advanced) {
  ...
  if (!advanced) {
    ...
    html += `<div class="gen-params-hint">Video animates a start frame (or text-only when the engine allows). It does not apply Image-tab LoRAs.</div>`;
  } else {
...
function _buildParamsPayload(kind, effective, promptText) {
  ...
  if (kind === 'video') {
    ...
    // the connected Comfy catalog's `video_models` section now -- no
  ...
  // else: untouched — omit `loras` so the backend fills them from the
  // selected model's own bundled LoRAs (apply_model_preset()).
